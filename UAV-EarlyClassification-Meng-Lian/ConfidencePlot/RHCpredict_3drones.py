# This code is used for testing the trained RHC. 
import torch
...
test_loader = torch.utils.data.DataLoader(dataset=test_dataset0,
                                          batch_size=33,                      # batch_size设置为33（即test数据集中instance的数量），将所有测试数据一次性输入模型进行预测，比较推荐此种做法
                                          shuffle=False)
...
with torch.no_grad(): 
    for k, (data, labels) in enumerate(test_loader):
        ...
        percentage_observed = 0.1            # the percentage of time steps that has observed
